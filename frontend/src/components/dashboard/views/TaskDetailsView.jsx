...
const TaskDetailsView = () => {
    ...
    if (localLoading)
        return <div className="flex justify-center items-center h-64 text-gray-600 text-lg">Loading task details...</div>;
    if (!task)
        return <div className="flex justify-center items-center h-64 text-gray-600 text-lg">Task not found</div>;
    ...
    return (
        <div className="max-w-4xl mx-auto p-6 space-y-6">
            ...
            <div className="bg-white rounded-xl shadow-lg border border-gray-200 p-8 space-y-6">
                <div>
                    {!requestSent ? (
                        <button className="px-6 py-3 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium" onClick={() => setShowRequestForm(true)}>
                            Send Request
                        </button>
                    ) : (
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                    ...
                    <div className="space-y-4">
                        ...
                        <div>
                            ...
                            <img 
                                src={(task.picture && task.picture.trim() !== '') ? (task.picture.startsWith('http') ? task.picture : `http://localhost:2000${task.picture}`) : `https://picsum.photos/seed/${task._id}/400/200`} 
                                onError={(e) => { e.target.onerror = null; e.target.src = `https://picsum.photos/seed/${task._id}/400/200`; }}
                                alt={task.title} 
            ...
            <SendRequestForm taskId={task._id} taskTitle={task.title} isOpen={showRequestForm} onClose={() => setShowRequestForm(false)} onSuccess={handleSendRequestSuccess}/>
        </div>
    );
};
