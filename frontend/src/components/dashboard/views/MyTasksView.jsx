...
const MyTasksView = () => {
    ...
    useEffect(() => {
        setFilteredTasks(myTasks.filter((task) => task.title.toLowerCase().includes(searchTerm.toLowerCase()) ||
            ...
            {filteredTasks.length === 0 ? (
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4 gap-6">
                    {filteredTasks.map((task) => (
                        <div key={task._id} className="bg-white rounded-xl shadow-sm border border-gray-100 overflow-hidden hover:shadow-md transition-shadow duration-200 flex flex-col h-full">
                            <div className="w-full h-40 shrink-0 overflow-hidden">
                                <img 
                                    src={(task.picture && task.picture.trim() !== '') ? (task.picture.startsWith('http') ? task.picture : `http://localhost:2000/${task.picture.replace(/^\/+/, '')}`) : `https://picsum.photos/seed/${task._id}/400/400`} 
                                    onError={(e) => { e.target.onerror = null; e.target.src = `https://picsum.photos/seed/${task._id}/400/400`; }}
                                    alt={task.title} 
                            ...
                            <div className="p-3 flex flex-col justify-between flex-1">
                                ...
                                <div className="flex space-x-1 mt-2">
                                    <button onClick={() => navigate(`/tasks/edit/${task._id}`)} className="flex-1 bg-gray-100 hover:bg-gray-200 text-gray-700 font-medium py-1 rounded-lg text-xs">
                                        Edit
                                    </button>
