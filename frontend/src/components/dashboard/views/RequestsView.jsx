...
const RequestsView = () => {
    ...
    const handleAcceptRequest = async (requestId) => {
        try {
            ...
            setReceivedRequests(prev => prev.map(r => r._id === requestId ? { ...r, status: 'accepted' } : r));
            setFilteredRequests(prev => prev.map(r => r._id === requestId ? { ...r, status: 'accepted' } : r));
    ...
    const handleRejectRequest = async (requestId) => {
        try {
            ...
            setReceivedRequests(prev => prev.map(r => r._id === requestId ? { ...r, status: 'rejected' } : r));
            setFilteredRequests(prev => prev.map(r => r._id === requestId ? { ...r, status: 'rejected' } : r));
    ...
    const handleViewApplications = async (taskId) => {
        try {
    ...
    useEffect(() => {
        ...
        setFilteredRequests(receivedRequests.filter(r => (r.task_id?.title || '').toLowerCase().includes(term) ||
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-3">
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          <input type="text" placeholder="Search by task, location, or applicant..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} onKeyDown={handleKeyPress} className="px-3 py-2 border rounded-lg flex-1 focus:outline-none focus:ring-2 focus:ring-blue-400"/>
          <button onClick={() => { }} // Search is handled by useEffect
     className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium">
      ...
      <div className="space-y-4">
        {loading ? (<div className="flex justify-center items-center h-32">
          ...
          </div>) : apiError ? (<div className="text-red-600">
            ...
            <div className="text-gray-500">
          ...
          </div>) : (filteredRequests.map((request) => (<div key={request._id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow duration-200">
              ...
              <div className="flex space-x-3 items-center justify-between">
                <div className="flex space-x-3">
                  {request.status === 'pending' && (<>
                      <button className="px-4 py-2 border border-green-200 text-green-600 hover:bg-green-50 font-medium rounded-lg transition-colors" onClick={() => handleAcceptRequest(request._id)}>
                        Accept
                      </button>
                      <button className="px-4 py-2 border border-red-200 text-red-600 hover:bg-red-50 font-medium rounded-lg transition-colors" onClick={() => handleRejectRequest(request._id)}>
                        Reject
                      </button>
                    </>)}
                  <span className={`px-3 py-1 rounded-full text-sm font-medium ${request.status === 'accepted' ? 'bg-green-100 text-green-800' :
                ...
                <button className="flex items-center px-4 py-2 bg-blue-600 hover:bg-blue-700 text-white font-medium rounded-lg transition-colors" onClick={() => handleViewApplications(request.task_id?._id)}>
              ...
              {/* Applications List */}
              {showAppsFor === request.task_id?._id && applications[request.task_id?._id] && (<div className="mt-4 bg-gray-50 rounded-lg p-4">
                  <h4 className="font-semibold mb-2 flex items-center">
                    <Users className="h-4 w-4 mr-2"/>
                    All Applications for this Task:
                  </h4>
                  {applications[request.task_id._id].length === 0 ? (<div className="text-gray-500">No other applications found.</div>) : (<ul className="space-y-3">
                      {applications[request.task_id._id].map((app) => (<li key={app._id} className="p-3 bg-white rounded border">
