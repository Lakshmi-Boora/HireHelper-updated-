...
const LandingPage = () => {
    ...
    return (<div className="min-h-screen flex flex-col bg-gradient-to-br from-blue-50 via-white to-indigo-50 relative overflow-x-hidden">
      ...
      <nav className="w-full flex justify-between items-center px-4 sm:px-8 py-4 bg-white/70 shadow-sm border-b border-gray-100 sticky top-0 z-30 backdrop-blur-xl transition-all">
        <div className="flex items-center gap-2 cursor-pointer" onClick={() => window.location.href = '/'} tabIndex={0} aria-label="Go to home" onKeyDown={e => { if (e.key === 'Enter') {
        ...
        <div className="flex gap-2 sm:gap-4 items-center">
          <button className={`text-gray-700 font-medium hover:text-blue-600 transition px-2 py-1 rounded focus:outline-none focus:ring-2 focus:ring-blue-400 ${activeSection === 'features' ? 'bg-blue-100 text-blue-700' : ''}`} onClick={() => featuresRef.current?.scrollIntoView({ behavior: 'smooth' })} aria-current={activeSection === 'features' ? 'page' : undefined}>Features</button>
          <button className={`text-gray-700 font-medium hover:text-blue-600 transition px-2 py-1 rounded focus:outline-none focus:ring-2 focus:ring-blue-400 ${activeSection === 'testimonials' ? 'bg-blue-100 text-blue-700' : ''}`} onClick={() => testimonialsRef.current?.scrollIntoView({ behavior: 'smooth' })} aria-current={activeSection === 'testimonials' ? 'page' : undefined}>Testimonials</button>
          <button className={`text-gray-700 font-medium hover:text-blue-600 transition px-2 py-1 rounded focus:outline-none focus:ring-2 focus:ring-blue-400 ${activeSection === 'contact' ? 'bg-blue-100 text-blue-700' : ''}`} onClick={() => contactRef.current?.scrollIntoView({ behavior: 'smooth' })} aria-current={activeSection === 'contact' ? 'page' : undefined}>Contact</button>
          <button className="px-5 py-2 bg-blue-600 text-white rounded-xl font-semibold shadow hover:scale-105 hover:shadow-lg hover:bg-blue-700 transition-all duration-200 text-base focus:outline-none focus:ring-2 focus:ring-blue-400" onClick={() => navigate('/login')}>Login</button>
          <button className="px-5 py-2 border border-green-600 text-green-600 rounded-xl font-semibold shadow hover:bg-green-50 hover:scale-105 transition-all duration-200 text-base focus:outline-none focus:ring-2 focus:ring-green-400" onClick={() => navigate('/login', { state: { signup: true } })}>Sign Up</button>
        </div>
      </nav>
        ...
        <div className="max-w-7xl w-full min-h-[540px] flex flex-col md:flex-row items-center justify-between gap-8 md:gap-14 bg-white/70 rounded-3xl shadow-2xl border border-gray-100 p-4 sm:p-8 md:p-14 backdrop-blur-lg z-10 relative">
          ...
          <div className="flex-1 flex flex-col items-start justify-center">
            ...
            <div className="flex gap-4">
              <button className="px-8 py-3 bg-blue-600 text-white font-semibold rounded-xl shadow-lg hover:scale-105 hover:shadow-xl hover:bg-blue-700 transition-all duration-200 text-lg focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2" onClick={() => navigate('/login')} tabIndex={0} aria-label="Get Started">
                Get Started
              </button>
              <button className="px-8 py-3 border border-blue-600 text-blue-600 font-semibold rounded-xl shadow hover:bg-blue-50 hover:scale-105 transition-all duration-200 text-lg focus:outline-none focus:ring-2 focus:ring-blue-400 focus:ring-offset-2" onClick={() => navigate('/login', { state: { signup: true } })} tabIndex={0} aria-label="Sign Up">
                Sign Up
              </button>
            </div>
          </div>
      ...
      <section ref={featuresRef} className="py-16 px-2 sm:px-4 bg-gradient-to-r from-blue-50 to-indigo-50 border-t border-b border-gray-100" id="features">
        ...
        <div className="max-w-5xl mx-auto grid grid-cols-1 sm:grid-cols-2 md:grid-cols-4 gap-8">
          {features.map((feature, i) => (<div key={feature.title} className={`feature-card bg-white rounded-2xl shadow p-6 flex flex-col items-center text-center border border-gray-100 transition-transform duration-300 hover:scale-105 hover:shadow-xl group opacity-0`} style={{ transitionDelay: `${i * 80}ms` }} tabIndex={0} aria-label={feature.title}>
      ...
      <section ref={testimonialsRef} className="py-16 px-2 sm:px-4 bg-white border-b border-gray-100" id="testimonials">
        ...
        <div className="max-w-4xl mx-auto grid grid-cols-1 md:grid-cols-3 gap-8">
          {testimonials.map((t, idx) => (<div key={idx} className="bg-blue-50 rounded-2xl shadow p-6 flex flex-col items-center border border-blue-100">
      ...
      {showScroll && (<button className="fixed bottom-6 right-6 z-50 bg-blue-600 text-white p-3 rounded-full shadow-lg hover:bg-blue-700 hover:scale-110 transition-all focus:outline-none focus:ring-2 focus:ring-blue-400" onClick={() => window.scrollTo({ top: 0, behavior: 'smooth' })} aria-label="Scroll to top">
