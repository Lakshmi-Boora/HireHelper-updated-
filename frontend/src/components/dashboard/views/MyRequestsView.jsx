...
const MyRequestsView = () => {
    ...
    useEffect(() => {
        ...
        setFilteredRequests(myRequests.filter(r => (r.task_id?.title || '').toLowerCase().includes(term) ||
    ...
    return (<div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-center gap-3">
        ...
        <div className="flex gap-2 w-full sm:w-auto">
          <input type="text" placeholder="Search by task title or location..." value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} onKeyDown={handleKeyPress} className="px-3 py-2 border rounded-lg flex-1 focus:outline-none focus:ring-2 focus:ring-blue-400"/>
     ...
     className="px-4 py-2 bg-blue-600 text-white rounded-lg hover:bg-blue-700 transition-colors font-medium">
      ...
      {loading ? (<div className="flex justify-center items-center h-32">
          ...
          <div className="text-gray-500">
            ...
            return (<div key={request._id} className="bg-white rounded-xl shadow-sm border border-gray-100 p-6 hover:shadow-md transition-shadow duration-200">
              ...
              <div className="flex space-x-3 items-center">
                {request.status === 'pending' && (<button className="flex items-center px-4 py-2 border border-red-200 text-red-600 hover:bg-red-50 font-medium rounded-lg transition-colors" onClick={() => handleDeleteRequest(request._id)}>
